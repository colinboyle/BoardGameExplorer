...
class NewReleasesBloc {
  ...
  StreamSink<ApiResponse<List<NewRelease>>> get newReleasesSink =>
      _newReleasesController.sink;
  Stream<ApiResponse<List<NewRelease>>> get newReleasesStream =>
      _newReleasesController.stream;
  ...
  Future<List<BoardGame>> fetchNewReleaseData(NewReleases releases) async {
    try {
      String gameIDs =
          releases.items.map((e) => e.parent.objectid + ',').join();
      //print('in bloc fetching full list with ids');
      //print(gameIDs);
      List<BoardGame> boardGames =
          await _newReleasesRepository.fetchNewReleasesDataList(gameIDs);
      return boardGames;
    } catch (e) {
